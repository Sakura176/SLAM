...
// global variables
string first_file = "../../left.jpg";
string second_file = "../../right.jpg";

// 32 bit unsigned int, will have 8, 8x32=256
typedef vector<uint32_t> DescType; // Descritor type

/**
 * compute descriptor of orb keypoints
 * @param img input image
 * @param keypoings detected fasst keypoints
 * @param descriptors descripyors
 * 
...
{
    // load image
    cv::Mat first_image = imread(first_file, 0);
    cv::Mat second_image = imread(second_file, 0);
    resize(first_image, first_image, Size(0, 0), 0.3, 0.3, INTER_AREA);
    resize(second_image, second_image, Size(0, 0), 0.3, 0.3, INTER_AREA);
